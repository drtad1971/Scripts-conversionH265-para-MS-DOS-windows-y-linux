...
cls
echo %B%================================================================%N%
echo   DETECTANDO HARDWARE V4.1.0 - TEST DE COMPATIBILIDAD REAL 10-BIT
echo %B%================================================================%N%
...
for /f "usebackq tokens=*" %%g in ("%temp%\gpus.tmp") do (
    ...
    echo !gpu_current! | findstr /i "NVIDIA" >nul && (
        "%VarPath%\ffmpeg.exe" -f lavfi -i color=c=black:s=64x64:n=1 -pix_fmt yuv420p10le -vframes 1 -c:v hevc_nvenc -f null - >nul 2>&1
        if !errorlevel! equ 0 (set "has_nv=1" & set "status=[OK 10-BIT]")
    )
    echo !gpu_current! | findstr /i "AMD ATI" >nul && (
        "%VarPath%\ffmpeg.exe" -f lavfi -i color=c=black:s=64x64:n=1 -pix_fmt yuv420p10le -vframes 1 -c:v hevc_amf -f null - >nul 2>&1
        if !errorlevel! equ 0 (set "has_amd=1" & set "status=[OK]")
    )
    echo !gpu_current! | findstr /i "Intel" >nul && (
        "%VarPath%\ffmpeg.exe" -f lavfi -i color=c=black:s=64x64:n=1 -pix_fmt yuv420p10le -vframes 1 -c:v hevc_qsv -f null - >nul 2>&1
...
for %%e in (mkv mp4 mov avi mpeg mpg ogv) do (
    for %%f in ("%origen%\*.%%e") do (
        ...
        for /f %%a in ('powershell -NoProfile -Command "[math]::round((Get-Item -LiteralPath '%%f').Length / 1MB, 2)"') do set "size_orig=%%a"
        set "t_inicio=!time!"
        echo ARCHIVO: !archivo! >> "%reporte%"
        echo - Inicio: !t_inicio! >> "%reporte%"
        echo - Tamano Original: !size_orig! MB >> "%reporte%"

        "%VarPath%\ffmpeg.exe" -i "%%f" -an -sn -pix_fmt yuv420p10le -c:v %codec% %params% -profile:v main10 -level:v 5.1 "%destino%\video_!archivo!.mkv" -y
        "%VarPath%\mkvmerge.exe" -o "%destino%\!purename!.mkv" -D "%%f" -A -S -T -M -B --no-chapters "%destino%\video_!archivo!.mkv"
        ...
        for /f "tokens=1,2,3" %%a in ('powershell -NoProfile -Command "$i=[datetime]::Parse('!t_inicio!'.Trim().Replace(',','.')); $f=[datetime]::Parse('!t_fin!'.Trim().Replace(',','.')); if($f -lt $i){$f=$f.AddDays(1)}; $dur=$f-$i; $ahorro=[math]::round(!s_orig_ps! - !s_final_ps!, 2); $perc=[math]::round(($ahorro/!s_orig_ps!)*100, 2); write-host ($dur.ToString('hh\:mm\:ss') + ' ' + $ahorro + ' ' + $perc)"') do (
            set "duracion=%%a"
            set "ahorro_mb=%%b"
            set "porcentaje=%%c"
        )
